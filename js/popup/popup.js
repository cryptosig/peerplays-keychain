...
function voteFor(name) {
  if (activeAccount.hasKey("active")) {
    ...
    peerplays.broadcast.accountWitnessVote(
...
async function sendTransfer() {
  ...
  if ((memo != "" && $("#encrypt_memo").prop("checked")) || memo[0] == "#") {
    try {
      const receiver = await peerplays.api.getAccountsAsync([to]);
  ...
  if (to != "" && amount != "" && amount >= 0.001) {
    peerplays.broadcast.transfer(
      ...
      async function(err, result) {
        ...
        if (err == null) {
          const sender = await peerplays.api.getAccountsAsync([
            activeAccount.getName()
          ]);
