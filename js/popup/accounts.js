...
const showUserData = async () => {
  ...
  showBalances();
  const balance =
    $("#currency_send .select-selected").text() === "PPY"
      ? await activeAccount.getPPY()
...
$("#check_add_account").click(function () {
  ...
  if (username !== "" && pwd !== "") {
    if (accountsList && accountsList.get(username)) {
      ...
      peerplays.api.getAccounts([username], function (err, result) {
        if (result.length != 0) {
          const active_info = result["0"].active;
          const posting_info = result["0"].posting;
          const pub_memo = result["0"].memo_key;
          if (peerplays.auth.isWif(pwd)) {
            const pub_unknown = peerplays.auth.wifToPublic(pwd);
            ...
            const keys = peerplays.auth.getPrivateKeys(username, pwd, [
              "owner",
              "active",
              "memo",
            ]);
...
$("#save_master").click(function () {
  if (
    ...
    const keys = peerplays.auth.getPrivateKeys(
      $("#username").val(),
      $("#pwd").val(),
      permissions
    );
...
const manageKeys = (name) => {
  ...
  $("#add_new_key")
    ...
    .click(function () {
      ...
      peerplays.api.getAccounts([name], function (err, result) {
        if (result.length != 0) {
          const active_info = result["0"].active;
          const posting_info = result["0"].posting;
          const pub_memo = result["0"].memo_key;
          if (peerplays.auth.isWif(pwd)) {
            ...
            const keys = peerplays.auth.getPrivateKeys(name, pwd, [
              "posting",
              "active",
              "memo",
            ]);
...
const showBalances = async () => {
  $("#wallet_amt div")
    .eq(0)
    .html(numberWithCommas(await activeAccount.getPPY()));
  $("#wallet_amt div")
    .eq(1)
