...
const setupInjection = () => {
    try {
        ...
        scriptTag.src = chrome.runtime.getURL("js/peerplays_keychain.js");
        var container = document.head || document.documentElement;
        container.insertBefore(scriptTag, container.children[0]);
    } catch (e) {
        console.error("Peerplays Keychain injection failed.", e);
    }
};
setupInjection();

// Answering the handshakes
document.addEventListener("swHandshake_peerplays", function (request) {
    ...
    if (request.detail.extension)
        ...
        window.postMessage(
            {
                type: "peerplays_keychain_handshake",
            },
...
document.addEventListener("swRequest_peerplays", function (request) {
    const prevReq = req;
    req = request.detail;
...
const sendResponse = (response) => {
    if (response.data.extension && response.data.extensionName) {
        ...
        window.postMessage(
            {
                type: "peerplays_keychain_response",
                response,
            },
            window.location.origin
        );
...
const isFilledDelegationMethod = (obj) => {
    return obj === "VESTS" || obj === "HP"; // todo: no hp in peerplays
};
//likely doesnt apply to peerplays
...
//this needs a rewrite for peerplays
...
const isFilledAmtpBTC = (amt) => {
    return (
        amt &&
        amt.split(" ").length === 2 &&
        !isNaN(amt.split(" ")[0]) &&
        parseFloat(countDecimals(amt.split(" ")[0])) === 8 &&
        amt.split(" ")[1] === "pBTC"
    );
};
...
const isFilledCurrency = (obj) => {
    return isFilled(obj) && (obj === "HIVE" || obj === "pBTC");
};

const isFilledKey = (obj) => {
    return (
        isFilled(obj) && (obj === "Memo" || obj === "Active" || obj === "Owner")
    );
};
