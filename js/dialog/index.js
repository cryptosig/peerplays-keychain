...
chrome.runtime.onMessage.addListener(function(
  ...
  if (command === "sendDialogError") {
    ...
    switch (type) {
      ...
      case "powerUp":
        $("#to").text(`@${recipient}`);
        $("#amount").text(`${steem} PPY`);
        break;
      case "powerDown":
        showBalances(username, "gpos", steem_power);
        $("#amount").text(`${steem_power} gpos`);
        break;
      case "createProposal":
        $("#receiver").text(receiver);
...
const showBalances = async (user, currency, amount) => {
  let balance = 0;
  hive.api.setOptions({ url: "https://api.mainnet.peerblock.trade/" });
  if (["ppy", "gpos"].includes(currency.toLowerCase())) {
    const account = (await peerplays.api.getAccountsAsync([user]))[0];
    switch (currency.toLowerCase()) {
      case "ppy":
        balance = parseFloat(account.balance.split(" ")[0]);
        break;
      case "gpos":
        balance = await getGposPower(account.vesting_shares);
        break;
    }
  } else {
...
//TODO: this needs work, NO VESTTOSTEEM
const getGposPower = async vesting_shares => {
  const result = await peerplays.api.getDynamicGlobalPropertiesAsync();
